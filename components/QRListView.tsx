...
export const QRListView: React.FC<QRListViewProps> = ({ qrs, onDelete, onSimulateScan }) => {
  ...
  const handleDelete = async (id: string) => {
    if (confirm('Eliminare definitivamente questo QR dal cloud?')) {
      setIsDeleting(id);
      try {
  ...
  if (qrs.length === 0) {
    return (
      <div className="text-center py-12 bg-white rounded-2xl border border-dashed border-slate-300">
        <p className="text-slate-500">Database vuoto. Crea il tuo primo QR!</p>
      </div>
    );
  }
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
      {qrs.map((qr) => (
        <div key={qr.id} className={`bg-white p-6 rounded-2xl shadow-sm border border-slate-100 hover:shadow-md transition-all ${isDeleting === qr.id ? 'opacity-50 grayscale' : ''}`}>
          ...
          <div className="flex justify-center mb-6 bg-slate-50 p-4 rounded-xl cursor-pointer group relative" onClick={() => setSelectedQR(qr)}>
            <QRCodeCanvas value={getTrackingUrl(qr.id)} size={140} />
            <div className="absolute inset-0 bg-white/60 opacity-0 group-hover:opacity-100 flex items-center justify-center transition-opacity rounded-xl font-bold text-blue-600 text-sm">
          ...
          <div className="space-y-3">
            ...
            >
              {copiedId === qr.id ? 'URL Copiato!' : 'Copia Link Pubblico'}
            </button>
            <button 
      ...
      {selectedQR && (
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={() => setSelectedQR(null)}>
          <div className="bg-white rounded-3xl max-w-md w-full p-8 relative shadow-2xl" onClick={e => e.stopPropagation()}>
            <button onClick={() => setSelectedQR(null)} className="absolute top-4 right-4 p-2 hover:bg-slate-100 rounded-full">
            ...
            <div className="text-center">
              ...
              <div className="bg-white p-6 rounded-2xl shadow-inner border border-slate-100 inline-block mb-6">
                <QRCodeCanvas id="download-qr" value={getTrackingUrl(selectedQR.id)} size={250} />
              </div>
              <button 
                onClick={() => {
                   const canvas = document.getElementById('download-qr') as HTMLCanvasElement;
